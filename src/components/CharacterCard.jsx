...
const CharacterCard = ({ character, isFavorite }) => {
  const toggleCharacterId = (e, characterId) => {
    e.preventDefault();

    const charactersIds = Cookies.get("characterIds") || "";
    const updatedIds = charactersIds
      .split(",")
      .filter((id) => id !== characterId)
      .join(",");

    if (charactersIds.includes(characterId)) {
      Cookies.set("characterIds", updatedIds, { expires: 7 });

      e.target.classList.remove("filled-red");
    } else {
      const updatedIdsWithCharacter = updatedIds
        ? `${updatedIds},${characterId}`
        : characterId;
      Cookies.set("characterIds", updatedIdsWithCharacter, { expires: 7 });
  ...
  return (
    <div className="card-container">
      ...
      <svg
        ...
        onClick={(e) => toggleCharacterId(e, character._id)}
      >
